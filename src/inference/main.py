...
pickle.load(f)
def run_inference:
again read_from_kafka()
json_schema = StructType([StructField(), StructField])   schhema registry....basically
def add_features
fraud_predictions = prediction_def.filter(col(
fraud_predictions.selectExpr().writeStream.format('kafka').option.optio
